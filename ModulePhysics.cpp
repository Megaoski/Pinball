...
bool ModulePhysics::Start()
{
	...
	b2Body* tablero = world->CreateBody(&body);
	...
	fixture.shape = &shape;
	/*big_ball->CreateFixture(&fixture);*/ //DIBUJA LA BOLA DE ENMEDIO
...
update_status ModulePhysics::PreUpdate()
{
	...
	while (todelete.count() != 0)//deleting body
	{
...
PhysBody* ModulePhysics::CreateCircle(int x, int y, int radius, b2BodyType bodytype, bool bullet, float restitution)
{
	b2BodyDef body;
	body.type = bodytype;
	body.bullet = bullet;
...
PhysBody* ModulePhysics::CreateRectangleSensor(int x, int y, int width, int height, b2BodyType bodytype)
{
	b2BodyDef body;
	body.type = bodytype;
	body.position.Set(PIXEL_TO_METERS(x), PIXEL_TO_METERS(y));
...
PhysBody* ModulePhysics::CreatePolygon(int x, int y, int* points, int size, float density)
{
	b2BodyDef body;
	body.type = b2_staticBody;
	body.position.Set(PIXEL_TO_METERS(x), PIXEL_TO_METERS(y));
...
update_status ModulePhysics::PostUpdate()
{
	...
	if (clicked_body != nullptr && is_clicked)
	{
		b2MouseJointDef def;
		def.bodyA = ground;
	...
	if (is_clicked)
	{
		mouse_joint->SetTarget(mouse_position); // NO PINTA LA LINEA ROJA ?
	...
	if (App->input->GetMouseButton(SDL_BUTTON_LEFT) == KEY_UP && is_clicked)
	{
